...
export function StoreManagement({
  ...
  function checkIfEmptyForm(newData) {
    let allValuesUndefined = newData.every((element) => {
      return element.newValue === undefined; // Explicitly return the result of the comparison
    });

    return allValuesUndefined;
  }
  ...
  if (data) {
    const handleSubmit = async () => {
      ...
      let empty = checkIfEmptyForm(newData);

      if (empty) {
        setAlertInfo({
          show: true,
          message: "Please fill in a field!",
          type: "critical",
        });
      } else {
        const mutationDataList = newData.map((data) => ({
          ...
          orgUnit: "a1dP5m3Clw4",
          value:
            parseInt(newStock[data.id]) + parseInt(data.values[2].value) ||
            parseInt(data.values[2].value),
          managedBy: user.name,
        }));
        ...
        try {
          newDataToBeSendt =
            data.history.history !== undefined
              ? [dataForStore, ...data.history.history]
              : [dataForStore];
          await sendDataMutation({ history: newDataToBeSendt });
          setAlertInfo({
            show: true,
    ...
    return (
      <div>
        ...
        <NoticeBox className={classes.pad} title="Register New Commodities">
          This page is used to recieve and update the stock of the commodities.
          To recieve a commodity, enter the amount of the commodity you have
          recieved in the input field. If you have not recieved any of the
          commodities, leave the input field blank.
        </NoticeBox>
        ...
        <StockDataTable
          ...
          fieldText={"Enter amount recieved"}
        />
        {alertInfo.show && (
